...
### Step 2 — Set the flow ID in `.env.local`

Open `.env.local` and set:

```
NEXT_PUBLIC_DESCOPE_FLOW_LOGIN=sign-up-or-in-bank
```

That's the only change needed. The login page will now render the new passkey-first flow, showing a **"Sign in with Passkeys"** button alongside the email field:
...
**`app/dashboard/page.tsx`** — The passkey enrollment modal from UC3 has been removed. The `sign-up-or-in-bank` flow handles passkey enrollment inline at login, so the post-login modal is no longer needed.

| What | Why |
|------|-----|
| Passkey modal — removed | The new flow prompts for passkey enrollment inline at login |
| `useUser`, `useState`, `Descope` imports — removed | Nothing in the dashboard reads `freshlyMigrated` anymore |
...
All the code changes for UC7 are already in the `blackbox` branch. Here's what was added and why — open each file in your editor to follow along.
...
| Part | What it does |
|------|-------------|
| `authMiddleware` | Descope's built-in Next.js middleware — reads and validates the session JWT on every request, server-side |
...
**`app/dashboard/page.tsx`, `app/transfer/page.tsx`, `app/admin/page.tsx`**

Each protected page has two additions:

| What | Why |
|------|-----|
| `useSession()` + `isSessionLoading` guard | Prevents premature redirect while Descope hydrates the session from the cookie on page refresh |
| Removed `if (!user) router.replace("/")` | Middleware handles this server-side — the client-side check was redundant and caused the refresh bug |

**To verify:** Log out → navigate directly to `http://localhost:3000/dashboard` → you should be redirected to login immediately.
...
![DevTools cookies panel showing DS and DSR](screenshots/uc7-01-cookies-devtools.jpg)
...
![DS token decoded in jwt.io](screenshots/uc7-02-ds-jwt.jpg)
...
![DSR token decoded in jwt.io](screenshots/uc7-03-dsr-jwt.jpg)
...
![Session Management settings in Descope Console](screenshots/uc7-04-session-management-console.png)
